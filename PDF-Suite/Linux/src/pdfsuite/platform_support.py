...
PT-PT: Particularidades do Linux.
       ...
       Esta é a versão para Linux do PDF Suite. Não há aqui nenhuma
...
EN-UK: Linux specifics.
       ...
       This is the Linux version of PDF Suite. There is no operating-system
...
def poppler_present() -> bool:
    ...
    EN-UK: Whether poppler is available. It is what draws the page in the visual
           ...
           else works — hence optional rather than essential.
    """
...
def check_requirements(distro: Distro | None = None) -> list[Requirement]:
    ...
    return [
        ...
        Requirement(
            name="poppler",
            present=poppler_present(),
            essential=False,
            detail="Desenha a pagina no editor visual. Sem ele o editor abre em modo de lista.",
            command=install_command("poppler", familia),
        ),
    ]
