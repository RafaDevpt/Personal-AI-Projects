...
PT-PT: Relatorios em HTML.
       ...
       por `escape()` antes de entrar no HTML. As mensagens do diario unificado
       contem caminhos, XML e, com alguma frequencia, sinais de menor e maior —
       as negacoes de sandbox trazem descritores inteiros entre parenteses
       angulares. A v1.0 inseria essas mensagens directamente e o resultado eram
...
EN-UK: HTML reports.
       ...
       One note worth more than it looks: everything coming from the system
       goes through `escape()` before entering the HTML. Unified log messages
       contain paths, XML and, fairly often, angle brackets. v1.0 inserted those
...
def _nome_seguro(texto: str) -> str:
    ...
    PT-PT: Reduz um texto a um nome de ficheiro seguro.
           ...
           Em macOS os dois-pontos merecem nota propria: o sistema aceita-os no
           nome, mas o Finder mostra-os como barras — heranca do Mac OS classico,
           onde os dois-pontos eram o separador de caminhos. Um relatorio chamado
           `saude:2026` aparece ao utilizador como `saude/2026` e ninguem o
           encontra. O filtro e o mesmo da versao de Windows, e serve tambem para
           estes ficheiros circularem por partilhas SMB sem se estragarem.

    EN-UK: Reduces text to a safe file name.

           On macOS the colon deserves its own note: the system accepts it in a
           name, but Finder displays it as a slash — inherited from classic Mac
           OS, where the colon was the path separator. A report called
           `health:2026` appears to the user as `health/2026` and nobody finds
           it.
    """
...
def _bloco_grupo(grupo) -> str:
    ...
    if regra:
        ...
        partes.append(
            ...
            "Esta mensagem não tem entrada na base de conhecimento. A repetição é o "
            "motivo de estar aqui. Ver o contexto com "
            "'log show --last 1h --predicate &#39;process == \"NOME\"&#39;'.</div>"
        )
...
def relatorio_eventos(analise: Analise, identificacao: dict[str, str]) -> str:
    ...
    if analise.outros:
        corpo.append("<h2>Outras mensagens registadas</h2>")
        corpo.append(
            "<table><tr><th>Processo</th><th>Tipo</th><th>Ocorrências</th>"
            "<th>Último</th><th>Mensagem</th></tr>"
        )
        for grupo in analise.outros[:60]:
            corpo.append(
                f"<tr><td>{escape(grupo.processo)}</td>"
                f"<td>{escape(grupo.nivel_texto)}</td>"
                f"<td>{grupo.contagem}</td><td>{escape(grupo.ultimo)}</td>"
                f"<td>{escape(grupo.exemplo)}</td></tr>"
            )
        corpo.append("</table>")
        if len(analise.outros) > 60:
            corpo.append(
                f'<div class="sub">Mais {len(analise.outros) - 60} tipo(s) de mensagem '
                "não listados.</div>"
            )
...
def relatorio_inventario(
    ...
    if actualizacoes:
        ...
        corpo.append("<tr><th>Pacote</th><th>Versão</th><th>Origem</th><th>Quando</th></tr>")
        for item in actualizacoes:
            corpo.append(
                f"<tr><td>{escape(str(item.get('pacote') or '?'))}</td>"
                f"<td>{escape(str(item.get('versao') or ''))}</td>"
                f"<td>{escape(str(item.get('accao') or ''))}</td>"
                f"<td>{escape(str(item.get('quando') or ''))}</td></tr>"
            )
        corpo.append("</table>")

    if software:
        corpo.append(f"<h2>Aplicações instaladas ({len(software)})</h2><table>")
        corpo.append("<tr><th>Nome</th><th>Versão</th><th>Origem</th></tr>")
        for item in software:
            corpo.append(
                f"<tr><td>{escape(str(item.get('nome') or ''))}</td>"
                f"<td>{escape(str(item.get('versao') or ''))}</td>"
                f"<td>{escape(str(item.get('origem') or ''))}</td></tr>"
            )
        corpo.append("</table>")
