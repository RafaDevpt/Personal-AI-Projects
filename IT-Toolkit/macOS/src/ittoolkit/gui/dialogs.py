...
class _JanelaBase(ctk.CTkToplevel):
    ...
    PT-PT: Base comum das janelas auxiliares.
           ...
           macOS, chamado imediatamente a seguir ao construtor, o CustomTkinter
    ...
    EN-UK: Common base for the auxiliary windows. The `grab_set` sits inside a
           200 ms `after` deliberately: on macOS, called straight after the
...
class JanelaDefinicoes(_JanelaBase):
    ...
    def _construir(self) -> None:
        ...
        # --- Diário -----------------------------------------------------------
        self._seccao(corpo, "Diário unificado")
        self.var_avisos = ctk.BooleanVar(value=self.config_app.incluir_avisos)
        self.var_paragens = ctk.BooleanVar(value=self.config_app.incluir_relatorios_paragem)

        ctk.CTkCheckBox(
            corpo,
            text="Incluir mensagens Default além dos erros (muito mais lento)",
            variable=self.var_avisos,
        ).pack(anchor="w", pady=theme.PAD_XS)
        ctk.CTkCheckBox(
            corpo,
            text="Ler os relatórios de paragem (exige Acesso Total ao Disco)",
            variable=self.var_paragens,
        ).pack(anchor="w", pady=theme.PAD_XS)

        self.max_eventos = self._campo_numero(
            corpo, "Máximo de registos a ler", self.config_app.max_eventos
        )
        self.dias_relatorios = self._campo_numero(
            corpo, "Dias de relatórios de paragem", self.config_app.dias_relatorios
        )
    ...
    def _gravar(self) -> None:
        ...
        config.incluir_relatorios_paragem = bool(self.var_paragens.get())
        config.dias_relatorios = self._inteiro(
            self.dias_relatorios, config.dias_relatorios, "Dias de relatórios"
        )
