...
class ITToolkitApp(ctk.CTk):
    ...
    def _barra_lateral(self) -> None:
        ...
        # PT-PT: As duas permissoes aparecem juntas de proposito. Num Mac sao
        #        independentes, e um operador que veja «root» assume que ve tudo
        #        — pode estar a correr como root sem Acesso Total ao Disco, e
        #        nesse caso metade do diario e todos os relatorios de paragem do
        #        sistema estao invisiveis sem um unico erro no ecra.
        # EN-UK: Both permissions appear together deliberately. On a Mac they are
        #        independent, and an operator seeing "root" assumes they see
        #        everything — they may be root without Full Disk Access, in which
        #        case half the log and every system crash report are invisible
        #        with not one error on screen.
        estado = "root" if self.ambiente.root else "sem privilégios"
        if not self.ambiente.acesso_total_ao_disco:
            estado += " · sem acesso total"
        ctk.CTkLabel(
            cabecalho,
    ...
    def _pagina_eventos(self, pagina: ctk.CTkFrame, etiqueta: str) -> None:
        self._titulo(
            ...
            "Lê, agrupa e interpreta o diário unificado, com causa provável e o "
            "que verificar.",
        )
        botoes = self._barra_botoes(pagina)
    ...
    def _pagina_servicos(self, pagina: ctk.CTkFrame, etiqueta: str) -> None:
        self._titulo(
            pagina, "Serviços",
            "Serviços do launchd cujo último arranque terminou com erro.",
        )
        botoes = self._barra_botoes(pagina)
        self._botao(botoes, "Listar problemas", self.listar_servicos, principal=True)

        self.entrada_servico = ctk.CTkEntry(
            botoes, placeholder_text="Etiqueta (com.exemplo.servico)", width=260,
            font=self.f_corpo,
        )
    ...
    def _pagina_ferramentas(self, pagina: ctk.CTkFrame, etiqueta: str) -> None:
        ...
        base = len(ACCOES)
        ctk.CTkLabel(
            grelha, text="Utilitários do sistema", font=self.f_seccao,
            text_color=theme.TEXT_MUTED, anchor="w",
        ).grid(
        ...
        # PT-PT: Quase todos estes utilitarios fazem parte do macOS, mas nem
        #        todos sobrevivem a todas as versoes — o Utilitario de Rede
        #        desapareceu no macOS 11. Um botao que nao abre nada e pior do
        #        que um botao que nao existe.
        # EN-UK: Nearly all these utilities ship with macOS, but not all survive
        #        every version — Network Utility went away in macOS 11.
    ...
    def analisar_eventos(self) -> None:
        """PT-PT: So o diario. / EN-UK: The log only."""
        config = self.config_app
        ...
        def trabalho():
            ...
            return events.analisar_maquina(
                config.periodo_horas, config.incluir_avisos, config.max_eventos,
            )

        # PT-PT: A mensagem diz «pode demorar» porque num Mac demora mesmo: o
        #        `log show` de 24 horas leva dezenas de segundos, e um operador
        #        que nao saiba disso carrega no botao outra vez.
        # EN-UK: The message says "may take a while" because on a Mac it does.
        self._trabalhar(trabalho, "eventos", "A ler o diário — pode demorar…")
    ...
    def _texto_eventos(self, analise) -> str:
        ...
        if analise.problemas:
            for grupo in analise.problemas:
                ...
                linhas.append(
                    f"{'':>15}{grupo.processo} · {grupo.nivel_texto} · "
        ...
        if analise.outros:
            ...
            for grupo in analise.outros[:25]:
                linhas.append(
                    f"  {grupo.nivel_texto:<13}{grupo.contagem:>4}×  "
                    f"{grupo.processo[:22]:<24}{grupo.exemplo[:60]}"
                )
        return "\n".join(linhas)
    ...
    def _receber_rede(self, resultado) -> None:
        ...
        for adaptador in adaptadores:
            linhas.append(
                f"{adaptador.get('descricao') or '?'}  ({adaptador.get('interface') or '?'})"
            )
            linhas.append(
                f"  IPv4        {adaptador.get('ipv4') or '?'}"
                f"  máscara {adaptador.get('mascara') or '?'}"
            )
    ...
    def _receber_discos(self, resultado) -> None:
        ...
        if not fisicos:
            linhas.append("  Sem dados. O diskutil não respondeu.")
    ...
    def _receber_servicos(self, lista) -> None:
        ...
        #        servicos, ou o texto de um registo. Aceitar as duas aqui evita
        #        uma segunda area de saida so para o registo, que e o mesmo
        #        assunto visto de mais perto.
        # EN-UK: This page receives two different things — a list of services, or
        #        a log's text.
        if isinstance(lista, str):
            self.escrever("servicos", lista)
            return
        if not lista:
            self.escrever("servicos", "Nenhum serviço do launchd com falha no último arranque.")
            return
        linhas = [f"{len(lista)} serviço(s) a precisar de atenção:", ""]
        for servico in lista:
            linhas.append(
                f"  {str(servico.get('dominio') or '?'):<12}"
                f"{str(servico.get('nome') or '?'):<44}"
                f"{str(servico.get('descricao') or '')}"
            )
        linhas.extend([
            "",
            "Escreva a etiqueta e carregue em Arrancar, ou em Ver registo para saber "
            "porque falhou.",
        ])
    ...
    def listar_servicos(self) -> None:
        def trabalho():
            ...
            return services.falhadas()

        self._trabalhar(trabalho, "servicos", "A ler os serviços…")

    def registo_servico(self) -> None:
        """PT-PT: Ultimas mensagens do diario de um servico.
        EN-UK: A service's last log messages."""
        nome = self.entrada_servico.get().strip()
        if not nome:
            messagebox.showinfo(__app_name__, "Escreva a etiqueta do serviço primeiro.")
            return
    ...
    def abrir_ferramenta_sistema(self, nome: str) -> None:
        from ..actions import abrir_ferramenta

        resultado = abrir_ferramenta(nome)
    ...
    def _receber_inventario(self, resultado) -> None:
        ...
        if actualizacoes:
            ...
            for item in actualizacoes:
                linhas.append(
                    ...
                    f"{str(item.get('pacote') or '?'):<40}"
                    f"{str(item.get('versao') or ''):<14}"
                    f"{item.get('accao') or ''}"
                )

        linhas.extend(["", f"APLICAÇÕES INSTALADAS ({len(software)})", ""])
        ...
        if not hardware and not software:
            linhas.append("  Sem dados. O system_profiler não respondeu.")
