...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        prog="ittoolkit",
        description=(
            "IT Toolkit — diagnóstico e manutenção de máquinas macOS. "
            f"{__credit__}"
        ),
        epilog=(
    ...
    parser.add_argument(
        "--sem-eventos",
        action="store_true",
        help="Salta a análise do diário. Num Mac é a opção que mais tempo poupa.",
    )
    parser.add_argument(
        "--sem-paragens",
        action="store_true",
        help="Não lê os relatórios de paragem em DiagnosticReports.",
    )
    parser.add_argument(
        "--sem-relatorio",
        action="store_true",
...
def executar_cli(args: argparse.Namespace, config: AppConfig) -> int:
    ...
    #        as a warning, not as "nothing found".
    if config.incluir_relatorios_paragem and not args.sem_paragens:
        from . import events
        from .models import Achado

        for entrada in events.relatorios_de_paragem(config.dias_relatorios):
            if entrada.get("sem_permissao"):
                print(
                    "[aviso] Sem Acesso Total ao Disco: os relatórios de paragem do "
                    f"sistema em {entrada['caminho']} não foram lidos."
                )
                continue
            achados.append(
                Achado(
                    modulo="Paragens",
                    titulo=f"{entrada['nome']} parou ({entrada['tipo']})",
                    detalhe=f"{entrada['quando']} — {entrada['caminho']}",
                    gravidade=Gravidade.ALTA,
                    solucao=(
                        "Abrir o ficheiro na Consola › Relatórios de Paragem. A secção "
                        "'Crashed Thread' identifica onde parou."
                    ),
                )
            )
        ...
        for grupo in analise.acionaveis:
            achados.append(
                Achado(
                    ...
                    detalhe=(
                        f"{grupo.contagem} ocorrência(s) em {grupo.processo}: "
                        f"{grupo.exemplo}"
                    ),
