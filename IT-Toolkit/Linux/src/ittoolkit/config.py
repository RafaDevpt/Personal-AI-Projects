...
class AppConfig:
    ...
    def diarios_escolhidos(self) -> list[str]:
        ...
        EN-UK: Which journals to read, according to the options.
               ...
               On Windows these are three logs with names of their own. Here
               they are two scopes of the same journal: the system's and the
               user's. They are not different files — they are different filters
               over the same journal, hence "scope" rather than "file".
        """
        escolhidos = []
        if self.incluir_sistema:
            escolhidos.append("sistema")
        if self.incluir_utilizador:
            escolhidos.append("utilizador")
        return escolhidos
