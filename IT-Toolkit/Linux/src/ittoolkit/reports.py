...
EN-UK: HTML reports.
       ...
       goes through `escape()` before entering the HTML. Journal messages contain
...
def _nome_seguro(texto: str) -> str:
    ...
    EN-UK: Reduces text to a safe file name.
           ...
           On Linux almost anything is valid in a filename — the slash and the
           null byte are the only real prohibitions. The filter is nonetheless
           the same as the Windows version's, deliberately: these reports travel
           by e-mail and end up on SMB shares, and a file called
           `report:2026.html` will not copy there.
    """
...
def _bloco_grupo(grupo) -> str:
    ...
    partes = [
        ...
        '<div class="meta">',
        f"{escape(grupo.unidade)} · {grupo.nivel_texto} · "
    ...
    if regra:
        ...
        partes.append(
            ...
            "'journalctl -u &lt;unidade&gt; --since -24h'.</div>"
        )
...
def relatorio_eventos(analise: Analise, identificacao: dict[str, str]) -> str:
    ...
    if analise.outros:
        ...
        corpo.append(
            "<table><tr><th>Unidade</th><th>Nível</th><th>Ocorrências</th>"
            "<th>Último</th><th>Mensagem</th></tr>"
        )
        for grupo in analise.outros[:60]:
            corpo.append(
                f"<tr><td>{escape(grupo.unidade)}</td>"
...
def relatorio_inventario(
    ...
    if actualizacoes:
        ...
        corpo.append("<tr><th>Pacote</th><th>Versão</th><th>Operação</th><th>Quando</th></tr>")
    ...
    if software:
        corpo.append(f"<h2>Pacotes instalados ({len(software)})</h2><table>")
