...
class JanelaDefinicoes(_JanelaBase):
    ...
    def _construir(self) -> None:
        ...
        # --- Diário ---------------------------------------------------------
        self._seccao(corpo, "Diário do systemd")
        self.var_sistema = ctk.BooleanVar(value=self.config_app.incluir_sistema)
        self.var_utilizador = ctk.BooleanVar(value=self.config_app.incluir_utilizador)
        self.var_arranque = ctk.BooleanVar(value=self.config_app.apenas_este_arranque)
        self.var_avisos = ctk.BooleanVar(value=self.config_app.incluir_avisos)

        ctk.CTkCheckBox(corpo, text="Diário do sistema", variable=self.var_sistema).pack(
            anchor="w", pady=theme.PAD_XS
        )
        ctk.CTkCheckBox(
            corpo, text="Diário deste utilizador (sessão gráfica)",
            variable=self.var_utilizador,
        ).pack(anchor="w", pady=theme.PAD_XS)
        ctk.CTkCheckBox(
            corpo, text="Apenas o arranque actual", variable=self.var_arranque
        ).pack(anchor="w", pady=theme.PAD_XS)
        ...
        self.max_eventos = self._campo_numero(
            corpo, "Máximo de registos a ler", self.config_app.max_eventos
        )
    ...
    def _gravar(self) -> None:
        ...
        config.incluir_sistema = bool(self.var_sistema.get())
        config.incluir_utilizador = bool(self.var_utilizador.get())
        config.apenas_este_arranque = bool(self.var_arranque.get())
        ...
        if not config.diarios_escolhidos:
            ...
            messagebox.showwarning(
                __app_name__,
                "Escolha pelo menos um diário para analisar. O do sistema foi "
                "reactivado.",
                parent=self,
            )
            config.incluir_sistema = True
