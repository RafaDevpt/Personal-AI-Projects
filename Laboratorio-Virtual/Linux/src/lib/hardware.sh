#!/usr/bin/env bash
# ===========================================================================
# PT-PT: Leitura das características da máquina anfitria, em Linux.
#
#        Tudo o que este ficheiro lê serve para responder a duas perguntas:
#        **esta máquina consegue virtualizar?** e **quanto pode dar sem se
#        prejudicar?**
#
#        Duas notas que poupam tempo a quem vier a seguir.
#
#        **Núcleos físicos não são `nproc`.** O `nproc` conta fios de execução,
#        e num processador com hyper-threading isso é o dobro dos núcleos. Dar
#        oito núcleos virtuais a partir de oito fios de quatro núcleos e dar o
#        dobro do que há, e o resultado e um convidado mais lento -- que é
#        exactamente o contrário do que quem escreveu o número queria. O
...
# ---------------------------------------------------------------------------
# PT-PT: Núcleos físicos. Ver o cabeçalho para o porque de não ser o `nproc`.
...
nucleos_fisicos() {
    ...
    if (( contagem <= 0 )) && [[ -r /proc/cpuinfo ]]; then
        ...
        #        interessa e o valor, não a contagem de linhas.
...
# ---------------------------------------------------------------------------
# PT-PT: Memória total, em MB. O `/proc/meminfo` da em kB.
...
# ---------------------------------------------------------------------------
# PT-PT: Espaço livre, em MB, no sistema de ficheiros que contém um caminho.
#
...
#        directório que exista, que é o que o `df` consegue medir.
#
...
# ---------------------------------------------------------------------------
# PT-PT: Se o processador anuncia extensões de virtualização.
...
# ---------------------------------------------------------------------------
# PT-PT: Se o KVM esta utilizável por este utilizador. Ver o cabeçalho.
#
#        Devolve: 0 utilizável · 1 não existe · 2 existe mas sem permissão
#
...
# ---------------------------------------------------------------------------
# PT-PT: O nome bonito da distribuição, do `/etc/os-release`.
...
# ---------------------------------------------------------------------------
# PT-PT: A família da distribuição, que é o que decide o gestor de pacotes.
#
#        O `ID_LIKE` e o que faz isto funcionar num Linux Mint ou num Pop!_OS
#        sem eles estarem em lista nenhuma: uma distribuição derivada preenche
...
#
#        Dizer "instale o qemu" a quem esta numa Fedora não ajuda; dizer
#        `sudo apt install` é pior, porque sugere que o programa não foi pensado
#        para o sistema dele.
#
...
# ---------------------------------------------------------------------------
# PT-PT: A arquitectura, no vocabulário do catálogo.
