...
#        do: ask where the file came from (macOS keeps both a Gatekeeper
#        quarantine flag and the download URL in an extended attribute); offer
...
formato_suportado() {
    ...
    case "$hipervisor:$extensao" in
        qemu:.iso|qemu:.qcow2|qemu:.qcow|qemu:.img|qemu:.raw|qemu:.vdi|qemu:.vmdk|qemu:.vhd|qemu:.vhdx)
            return 0 ;;
    ...
    case "$extensao" in
        .ova|.ovf)
            printf 'Uma appliance %s só se importa no VirtualBox — e num Mac com chip da Apple\n' "$extensao"
            printf 'nem isso, porque o VirtualBox não serve lá.\n'
...
assinatura_ficheiro() {
    ...
    local tamanho; tamanho="$(stat -f '%z' "$caminho" 2>/dev/null || printf '0')"
...
#        beats the other two: Gatekeeper quarantines everything downloaded, and
#        Spotlight stores the source URL alongside, in a binary plist that has
#        to go through `plutil` to be read.
#
...
origem_ficheiro() {
    ...
    command -v xattr >/dev/null 2>&1 || {
        printf 'O sistema não tem registo de onde este ficheiro veio.'
        return 1
    }
    ...
    #        most useful field macOS keeps about a file.
    local url=''
    if command -v plutil >/dev/null 2>&1 && command -v xxd >/dev/null 2>&1; then
        url="$(xattr -p com.apple.metadata:kMDItemWhereFroms "$caminho" 2>/dev/null \
               | xxd -r -p 2>/dev/null \
    ...
    # PT-PT: Sem o endereco, a quarentena do Gatekeeper ainda diz que **veio** de
    #        fora, e qual foi a aplicacao que o trouxe. E menos, mas nao e nada.
    # EN-UK: Without the URL, Gatekeeper's quarantine still says it **came** from
    #        outside, and which application brought it.
    local quarentena
    quarentena="$(xattr -p com.apple.quarantine "$caminho" 2>/dev/null || true)"
    if [[ -n "$quarentena" ]]; then
        local aplicacao; aplicacao="$(printf '%s' "$quarentena" | cut -d';' -f3)"
        if [[ -n "$aplicacao" ]]; then
            printf 'Este ficheiro foi descarregado pelo %s, mas o sistema não guardou de onde.' "$aplicacao"
        else
            printf 'Este ficheiro está em quarentena do Gatekeeper: veio de fora desta máquina.'
        fi
        return 0
    fi
